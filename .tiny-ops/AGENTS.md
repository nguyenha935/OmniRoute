...
6. Workflow phải cache npm và cache build Next.js thực tế ở `.build/next/cache` (không phải `.next/cache`), nhưng cache không được thay cổng typecheck/test/build. Candidate có cùng target, ordered patch identity, lane, runtime và dependency fingerprint phải được tái sử dụng thay vì build lại. Lane GitHub phải dùng Webpack có giới hạn bộ nhớ; Turbopack đã được đo đạt 15,1 GiB RSS cộng 3 GiB swap và làm chết hosted runner.
