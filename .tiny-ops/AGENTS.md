...
13. Full-package chỉ được chép các file runtime mà từng package `@omniroute/*` khai báo trong `package.json.files`; phải loại test/dev residue khỏi các runtime root trước khi tạo payload. `dist/node_modules` sinh từ standalone dev build là cây trùng và phải bỏ để runtime resolve lên `node_modules` production-pruned ở package root. Không được nới artifact policy để cho source test hoặc dev-only dependency lọt vào production.
14. Nếu exact upstream target đỏ sẵn ở `typecheck:core`, hosted builder phải chạy cùng gate trên target nguyên bản và candidate, chuẩn hóa diagnostics và chỉ cho qua khi candidate không thêm lỗi TypeScript mới. Không được bỏ typecheck hoặc dùng lỗi nền để che regression của patch.
15. `--update` phải backup package, data và config; package được đổi nguyên tử; health check lỗi hoặc tiến trình bị ngắt sau khi package thay đổi phải rollback.
16. Không chạy `npm install -g omniroute` thủ công để bỏ qua luồng update an toàn.
17. Không dùng `git stash` trong bất kỳ luồng OmniRoute nào.
