...
- Giữ nguyên hàng rào vận hành: `MemoryHigh=8G`, `MemoryMax=10G`,
  `MemorySwapMax=512M`, `CPUQuota=300%` và `CIRCLE_NODE_TOTAL=1`.
- Không build thủ công để thử lại. Chỉ chạy theo chuỗi có pin
  `--check` → `--preflight` → `--update` → `--verify-runtime` để giữ backup,
  smoke-test và rollback.
