...
# bounds only V8's JS heap, not webpack's native/worker allocations, which the Tiny
# incident logs measured at multiple GiB on top of the heap. A 14 GiB heap left too
# little headroom and the webpack overlay step was OOM-killed (exit 1). 8192 MB
# matches the upstream default and keeps total RSS well under the 16 GB ceiling.
readonly BUILD_MEMORY_MB="${OMNIROUTE_BUILD_MEMORY_MB:-8192}"
